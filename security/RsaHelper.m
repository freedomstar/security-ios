...
-(void)createKey
{
    ...
    //creating key pair
    [self createKeyPairWithTag:self.publicTag privateTag:self.privateTag];
...
{
    return [self getModulusWithTag:self.publicTag];
}

-(NSString*)getExponent
{
    return [self getExponentWithTag:self.publicTag];
}

-(NSString*)encryptionWith:(NSString*) cipherText
{
    return  [self decrypt:cipherText privateTag:self.privateTag];
}
